...
Load the package by calling `gapminder`. 
...
Plot a scatter plot of `lifeExp` as a function of `gdpPercap`. To do so, use the [same lines of code as in the previous exercise on airquality](ex_01_linear_regression.qmd), changing the relevant variables names. 
...
If instead you plot the distribution of `log(gdpPercap)` (using the previous lines of code), this distribution seems roughly `r fitb("normal", ignore_case = TRUE)`. Importantly, it is relatively cetered around its mean. 
...
$$y = \alpha + \beta x + e$$
What should be $y$ in your case? `r fitb("lifeExp")`. And x? `r fitb("log(gdpPercap)")`.
...
There are actually formal test for heteroskedasticity. We will not have time to cover them in my section of the class but 
